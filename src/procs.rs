...
impl ProcTable {
    ...
    pub fn sweep(&mut self) -> Vec<Completed> {
        ...
        if self.rows.len() > KEEP_AT_LEAST {
            ...
            self.rows.retain(|r| {
                let stale =
                    r.done_at.is_some_and(|t| now.duration_since(t) > KEEP_FINISHED);
...
mod tests {
    ...
    fn usage(tokens: i64, cost: Option<f64>) -> Usage {
        Usage { input_tokens: tokens, output_tokens: 0, cache_read_tokens: 0, cost_usd: cost }
    }
    ...
    fn stopping_one_page_leaves_the_others_running() {
        ...
        assert!(fix.snapshot().state.is_live(), "the fix session was not left");
    ...
    fn a_killed_calls_usage_stays_on_the_books() {
        ...
        a.ended(Ended::Cancelled("navigated away".into()), usage(1_000, Some(0.02)));
    ...
    fn a_finished_call_stops_counting_as_running_before_the_next_sweep() {
        ...
        assert_eq!(table.running_total(), 0, "no sweep has run yet, and it must not have to");
