...
fn the_summary_lays_out_both_ways_out_of_a_finished_review() {
    for (tag, behind) in [("summary-push", 0usize), ("summary-stack", 1)] {
        ...
        assert_eq!(frames[1], frames[2], "{tag}: the summary shifted between repaints");
    }
}
...
fn the_stop_confirmation_banner_lays_out_and_stays_put() {
    ...
    let handle = app.procs.register(crate::procs::Owner::Review, 0, "claude", "src/lib.rs:2");
    handle.started_process(31337);
    handle.set_session(Some("2f9a1c44-0000-4444-8888-aaaabbbbcccc".into()));
    app.goto(Screen::FilePicker);
    let notice = app.nav_notice.as_ref().expect("leaving a page with work says so");
    assert!(!notice.all_confirmed(), "nothing has confirmed the kill yet");
    ...
    // one row per process.
    handle.ended(crate::procs::Ended::Cancelled("left the review screen".into()), Default::default());
...
fn the_process_ledger_lays_out_every_state() {
    ...
    let running = app.procs.register(crate::procs::Owner::Review, 0, "claude", "src/lib.rs:2");
    running.started_process(101);
    running.set_session(Some("aaaa1111-2222".into()));
    let stopping = app.procs.register(crate::procs::Owner::Fix, 0, "codex", "3 note(s)");
    stopping.started_process(102);
    stopping.stop("left the follow-up screen");
    let done = app.procs.register(crate::procs::Owner::Branch, 0, "agy", "feature vs main");
    done.started_process(103);
    done.ended(crate::procs::Ended::Finished, Default::default());
    let never = app.procs.register(crate::procs::Owner::Review, 0, "claude", "src/lib.rs:4");
...
fn a_blinded_paused_card_gives_away_no_identifier() {
    ...
    assert!(call.session_line(true).contains("2f9a1c44-0000"), "{}", call.session_line(true));
    ...
    assert!(!blinded.contains("31337"), "the pid is a lookup away from the name: {blinded}");
    let session = call.session_line(false);
    assert!(!session.contains("2f9a1c44"), "the session id names the model too: {session}");
    assert!(session.contains("still open"), "the reviewer still needs to know it can resume: {session}");
...
fn a_paused_card_offers_resume_only_when_there_is_a_session() {
    ...
    assert!(!without.resumable(&app.settings), "there is nothing to continue");
    assert!(
        without.session_line(true).contains("no session id"),
        "{}",
        without.session_line(true)
    );
    assert!(with_session.line(true).contains("pid 31337 terminated"), "{}", with_session.line(true));
    ...
    assert_eq!(counts[3..], counts[2..counts.len() - 1], "the paused card never settled");
    ...
    assert!(matches!(app.candidates[0], crate::app::CandidateState::Paused(_)));
}
