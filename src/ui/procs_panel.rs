...
pub fn nav_notice(app: &mut CraApp, ctx: &egui::Context) {
    let Some(notice) = &app.nav_notice else { return };
        ...
        ui.horizontal_wrapped(|ui| {
            ...
            ui.label(
                RichText::new(notice.headline())
                    .strong()
                    .color(if settled { theme::GOOD } else { theme::WARN }),
            );
            ...
            ui.horizontal_wrapped(|ui| {
                ...
                match &r.session {
                    Some(id) => {
                        ...
                        ui.label(RichText::new(short(id)).monospace().small().color(theme::ACCENT))
                            .on_hover_text(format!("session {id}\nresume picks this conversation up"));
                    }
                    None => {
                        ui.label(theme::dim("· no session id — it was stopped before reporting one"));
                    }
                }
            });
        }
...
pub fn window(app: &mut CraApp, ctx: &egui::Context) {
        ...
        .show(ctx, |ui| {
            ...
            ui.horizontal_wrapped(|ui| {
                ...
                theme::badge(
                    ...
                    if running > 0 { theme::WARN } else { theme::TEXT_DIM },
                );
                ui.label(theme::dim(&format!("{} finished this run", app.procs.completed())));
                ...
                for row in app.procs.rows().iter().rev() {
                    ...
                    ui.horizontal_wrapped(|ui| {
                        ...
                        if snap.state.is_live()
                            && ui.small_button("⏹").on_hover_text("terminate this process").clicked()
                        {
                            stop = Some(row.id);
                        }
                    });
                    ...
                    for row in &earlier {
                        ui.horizontal_wrapped(|ui| {
                            ...
                            match row.cost_usd {
                                ...
                                _ => ui.label(
                                    RichText::new(format!("{} tok", row.tokens)).monospace().small(),
                                ),
                            };
                        });
                    }
    ...
    if stop_all {
        ...
        let n = receipts.len();
        if n > 0 {
            app.note("procs", &format!("stopping {n} process(es) from the ledger"));
        }
    }
    if !open {
...
pub fn paused_row(
    ...
    egui::Frame::none()
        ...
        .show(ui, |ui| {
            ...
            ui.horizontal_wrapped(|ui| {
                match (&call.session, view.identifying) {
                    (Some(id), true) => {
                        ui.label(theme::dim("session"));
                        ui.label(RichText::new(short(id)).monospace().small().color(theme::ACCENT))
                            .on_hover_text(format!("session {id}"));
            ...
            ui.horizontal_wrapped(|ui| {
                if ui
                    ...
                    .on_hover_text(match resumable {
                        true => "continues the same conversation — the model keeps what it had read",
                        false => "no session to continue: this CLI never reported one, or it has \
