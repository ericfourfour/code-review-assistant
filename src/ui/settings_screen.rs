...
than the other way round.",
        ...
        egui::Grid::new("discover_grid").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
            ui.label(theme::dim("max age (days)"));
            ui.horizontal(|ui| {
                ui.add(egui::DragValue::new(&mut self.settings.repo_max_age_days).range(0..=3650));
...
hidden; one that only exists on GitHub is cloned into the clone directory when opened.",
        ...
        if self.settings.excluded_repos.is_empty() {
            ...
            for (i, key) in self.settings.excluded_repos.iter().enumerate() {
                ui.horizontal(|ui| {
                    if ui.small_button("✕").on_hover_text("stop excluding").clicked() {
                        unexclude = Some(i);
                    }
